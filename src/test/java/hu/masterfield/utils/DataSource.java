...
public class DataSource {
    ...
    public static List<Saving> loadSavings() {
        ...
        List<Saving> savings = new CsvToBeanBuilder<Saving>(ls).withType(Saving.class).withSeparator(',').build().parse();
        return savings;
    }
    ...
    public static void saveSavings(List<Saving> savingList) {
        try (CSVWriter writer = new CSVWriter(new FileWriter(Consts.SAVE_SAVINGS_DATA_CSV))) {
            new StatefulBeanToCsvBuilder<>(writer).withOrderedResults(true).withApplyQuotesToAll(false).build().write(savingList);
        } catch (Exception ex) {
